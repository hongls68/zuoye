...
【第4周】自然语言查询与请求采集（把上面的接口封装成受限工具，交给运行时模型调用）：
  ...
  ★ 四条硬约束（都在 nl_agent.py 里，且有自测守着）：
      1. 受限工具：只能调白名单里的 9 个工具，没有自由写 SQL 的能力；
...
【第5周】传感器示波器：三轴波形 + 姿态孪生（对标参考产品的「传感器示波器」页）：
  ...
  ★ 这两块数据同样接进了第 4 周的自然语言问答（nl_agent.py）：
      get_attitude / query_waveform 两个**只读**工具。
    加的时候踩到过一处"自己给自己挖的坑"：新加一张表，只加接口是不够的 ——
      ① 设备白名单（known_devices）得跟着扫这张表，否则只传波形的设备
         会被判成"根本没这台设备"；
      ② 提示词里得补上口径，否则模型面对"板子朝哪边"只会留空或编个 0°。
    三处都补齐了，自测第 18~21 组盯着。
