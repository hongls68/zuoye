...
> 部署步骤、排错指南、课堂验收要点，见 **[`ai_week/README.md`](ai_week/README.md)**。
...
└── ai_week/                         ← 全部作业内容统一放这里（不分周次）
    ...
    ├── 项目复盘与踩坑记录.md         ← 第 1 周复盘：踩过的坑与可复用的排错方法论
    ├── 第二周作业-项目计划与四课实施方案（版本迭代稿）.md   ← 第 2 周主交付（v1.0 → v1.1）
    └── archive/                     ← 迭代过程中的单版本文档（历史留档）
```

> **路径约定**：全部作业统一放在 `ai_week/` 下，**不再按周次分子目录**。
...
| 文件 | 内容 | 对应周次 |
|---|---|---|
| [`ai_week/README.md`](ai_week/README.md) | 部署入口、运行说明、接口清单、排错 | 第 1 周 |
| [`ai_week/项目复盘与踩坑记录.md`](ai_week/项目复盘与踩坑记录.md) | 踩坑记录 + 分层排错方法论 | 第 1 周 |
| [`ai_week/摄像头功能方案与Git提交.md`](ai_week/摄像头功能方案与Git提交.md) | 摄像头功能实现方案 + Git 流程 | 第 1 周 |
| [`ai_week/第二周作业-项目计划与四课实施方案（版本迭代稿）.md`](ai_week/第二周作业-项目计划与四课实施方案（版本迭代稿）.md) | 项目计划书（v1.0）+ 四课实施与教学组织方案（v1.1） | 第 2 周 |
...
> 上表中 `93cd050` 一条的原文保留了当时的目录名 `ai-week1/`（提交时的真实路径）；
> 目录后经结构调整为 `ai_week/`，此处不改写历史提交信息。
...
- ⚠️ **目录结构调整后，`firmware/build/` 里的 CMake 缓存因绝对路径失效**，
  下次编译前请先 `idf.py fullclean`（或直接删除 `firmware/build/`）。
